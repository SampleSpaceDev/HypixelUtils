...
import dev.samplespace.hypixelutils.util.VerbalExpression;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

import static dev.samplespace.hypixelutils.util.VerbalExpression.regex;

public class PlayerJoinLeaveChatProcessor implements IChatProcessor {

    // ^(?<username>[0-9A-Za-z_]{1,16}) has joined \((?<count>[0-9]+)/(?<max>[0-9]+)\)!$
    private static final Pattern playerJoin = VerbalExpression.regex()
            .startOfLine()
    ...
    private static final Pattern playerLeave = VerbalExpression.regex()
            .startOfLine()
            .capture("username").word().count(1, 16).endCapture()
            .then(" has quit!")
            .buildPattern();
